...
Viewport::Viewport(int width, int height, sptr(Camera) camera)
    : m_width(width),
      m_height(height),
      m_camera(camera)
{
...
void Viewport::resize(int w, int h)
{
    ...
    // update cam
    m_camera->m_viewPlaneWidth = 2.0f;
    m_camera->m_viewPlaneHeight = 2.0f / m_aspect;

    m_camera->m_distance = 0.5f * m_camera->m_viewPlaneWidth * (1.0f / tan(math::DegToRad(m_camera->m_fov / 2.0f)));
}
